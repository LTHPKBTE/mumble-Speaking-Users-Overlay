...
- 可配置空闲超时 — 未发言用户超过 N 秒后自动从列表移除（默认 5 秒）
...
| **空闲超时(秒)** | 未发言用户超过此秒数后从列表移除 (1~30)，默认 5 |
| **显示窗口**（按钮） | 重新显示被隐藏的窗口 |
...
- 未发言用户：开启"显示未发言用户"后，未发言的用户会以较低透明度显示在列表中。连接服务器时会通过 Mumble API 获取**所有用户**，方便您看清频道里的所有人。
- 空闲超时：未发言用户超过 N 秒后自动从列表移除（可在设置中调节，默认 5 秒）。
